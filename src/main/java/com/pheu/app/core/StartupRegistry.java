...
/**
 * A registry that executes a set of commands. The registry will synchronously
 * execute commands when {@link #execute()} is invoked, returning early if any
 * action throws an exception. Only one call to {@link #execute()} will have an
 * effect, all subsequent calls will be ignored.
 */
public class StartupRegistry implements ExceptionalCommand{
	...
	public synchronized <E extends Exception, T extends ExceptionalCommand> void addAction(T action) {
