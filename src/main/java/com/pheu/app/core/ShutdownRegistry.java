...
public interface ShutdownRegistry {
   ...
   */
  <E extends Exception, T extends ExceptionalCommand> void addAction(T action);
  ...
  public static class ShutdownRegistryImpl implements ShutdownRegistry, Command {
    ...
    private final List<ExceptionalCommand> actions = Lists.newLinkedList();
     ...
     */
    @Override
    public synchronized <E extends Exception, T extends ExceptionalCommand> void addAction(
        T action) {
    ...
    public synchronized void execute() {
      if (!completed) {
        ...
        completed = true;
        try {
          for (ExceptionalCommand action : Lists.reverse(actions)) {
